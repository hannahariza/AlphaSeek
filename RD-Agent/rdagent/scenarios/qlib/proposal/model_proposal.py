...
class QlibModelHypothesis2Experiment(ModelHypothesis2Experiment):
    ...
    def convert_response(self, response: str, hypothesis: Hypothesis, trace: Trace) -> ModelExperiment:
        ...
        # Quant 场景：Model 需基于 Factor 实验获取 SOTA 因子
        # - FactorExperiment: 有 result 即纳入（供加载因子），不要求 decision
        # - ModelExperiment: 需 t[1] 通过（accepted）
        from rdagent.scenarios.qlib.experiment.factor_experiment import QlibFactorExperiment
        base_list = []
        for t in trace.hist:
            e, fb = t[0], t[1]
            if isinstance(e, QlibFactorExperiment) and getattr(e, "result", None) is not None:
                base_list.append(e)
            elif isinstance(e, ModelExperiment) and fb:
                base_list.append(e)
        exp.based_experiments = base_list
        return exp
