...
class QlibDataGenConf(LocalConf):
    """Configuration for qlib data generation using current conda environment"""
    default_entry: str = "python generate.py"
    enable_cache: bool = False
    bin_path: str = os.environ.get('PATH', '')


class QlibDataGenEnv(LocalEnv[QlibDataGenConf]):
    """Environment for running qlib data generation in current conda env"""
    pass


def generate_data_folder_from_qlib(use_docker: bool = True):
    """
    Generate data folder from qlib
    
    Args:
        use_docker: If True, use Docker; otherwise use local conda environment
    """
    ...
    if use_docker:
        try:
            ...
            print(f"Docker not available ({e}), falling back to conda environment")
            use_docker = False
    
    if not use_docker:
        # Use local conda environment (current activated env)
        conf = QlibDataGenConf()
        qtde = QlibDataGenEnv(conf=conf)
        qtde.prepare()
        current_env = os.environ.get('CONDA_DEFAULT_ENV', 'unknown')
        print(f"[green]Using conda environment: {current_env}[/green]")

    # Run the Qlib backtest with clean environment
    # Ensure we don't inherit QuantaAlpha's qlib configuration
    clean_env = {
        'PATH': os.environ.get('PATH', ''),
        'HOME': os.environ.get('HOME', ''),
        'CONDA_DEFAULT_ENV': os.environ.get('CONDA_DEFAULT_ENV', ''),
        'PYTHONPATH': '',  # Clear to avoid QuantaAlpha interference
    }
...
def get_data_folder_intro(fname_reg: str = ".*", flags=0, variable_mapping=None, use_docker: bool = True) -> str:
    """
    ...
    use_docker: bool
        If True, use Docker to generate data; otherwise use local conda environment
    ...
    if (
        ...
        generate_data_folder_from_qlib(use_docker=use_docker)
    content_l = []
