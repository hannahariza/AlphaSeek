...
# 启用 drop_params 自动丢弃不兼容的参数（修复 SiliconFlow 等国产模型兼容性问题）
...
class LiteLLMAPIBackend(APIBackend):
    ...
    def _create_embedding_inner_function(self, input_content_list: list[str]) -> list[list[float]]:
        ...
        # Build embedding kwargs with optional api_key and api_base for separate embedding provider
        embedding_kwargs: dict[str, Any] = {
            "model": model_name,
            "input": input_content_list,
            "encoding_format": "float",  # 修复 SiliconFlow 等国产模型兼容性问题
        }
        # Add embedding-specific API key and base URL if configured
