...
def value_evaluator(
    ...
    if prediction is None:
        ...
        # 无 ground truth 时视为通过：执行成功即可，与 factor 逻辑一致
        return (
            "No ground truth output provided. Value evaluation skipped; consider as passed when execution succeeds.",
            True,
        )
    else:
...
class ModelFinalEvaluator(CoSTEEREvaluator):
    def evaluate(
        ...
        # 兼容 LLM 可能返回的不同键名（如 Final Feedback、finalFeedback 等）
        final_feedback = final_evaluation_dict.get("final_feedback") or final_evaluation_dict.get(
            "Final Feedback"
        ) or final_evaluation_dict.get("finalFeedback") or str(final_evaluation_dict)
        final_decision = final_evaluation_dict.get("final_decision") or final_evaluation_dict.get(
            "Final Decision"
        ) or final_evaluation_dict.get("finalDecision")
        ...
        return (final_feedback, final_decision)
