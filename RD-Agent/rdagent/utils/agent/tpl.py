...
class RDAT:
    ...
    def r(self, **context: Any) -> str:
        ...
        env.filters["format_backtest_result"] = _format_backtest_result
        rendered = (
...
def _format_backtest_result(result: Any) -> str:
    """Format experiment.result for template (dict from LocalFactorRunner or DataFrame from Docker)."""
    if result is None:
        return "N/A"
    if hasattr(result, "loc"):
        try:
            return str(result.loc[["IC", "1day.excess_return_without_cost.annualized_return", "1day.excess_return_without_cost.max_drawdown"]])
        except Exception:
            return str(result)
    if isinstance(result, dict):
        ic = result.get("IC", result.get("ic", "N/A"))
        arr = result.get("annualized_return", result.get("1day.excess_return_without_cost.annualized_return", "N/A"))
        mdd = result.get("max_drawdown", result.get("1day.excess_return_without_cost.max_drawdown", "N/A"))
        return f"IC={ic}, ARR={arr}, MDD={mdd}"
    return str(result)
