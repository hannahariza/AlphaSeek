...
def test_embedding(embedding_model, embedding_api_key, embedding_api_base):
    ...
    logger.info(f"🧪 Testing embedding api_key: {embedding_api_key}")
    logger.info(f"🧪 Testing embedding api_base: {embedding_api_base}")
    # Debug: print all embedding-related env vars
    import os
    logger.info(f"DEBUG EMBEDDING_OPENAI_API_KEY from os.environ: {os.environ.get('EMBEDDING_OPENAI_API_KEY', 'NOT SET')[:20] if os.environ.get('EMBEDDING_OPENAI_API_KEY') else 'NOT SET'}")
    try:
        response = embedding(
...
def env_check():
    ...
    if "DEEPSEEK_API_KEY" in os.environ:
        ...
        # Support both new EMBEDDING_OPENAI_* and legacy LITELLM_PROXY_* variables
        embedding_api_key = os.getenv("EMBEDDING_OPENAI_API_KEY") or os.getenv("LITELLM_PROXY_API_KEY")
        embedding_api_base = os.getenv("EMBEDDING_OPENAI_API_BASE") or os.getenv("LITELLM_PROXY_API_BASE")
        ...
        embedding_api_base = os.getenv("EMBEDDING_OPENAI_API_BASE") or chat_api_base
    else:
