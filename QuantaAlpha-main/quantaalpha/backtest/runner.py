...
class BacktestRunner:
    ...
    def _create_dataset_with_computed_factors(self,
        ...
        # Fix: Ensure combined_df_multi index datetime level is Timestamp type and names are correct
        if isinstance(combined_df_multi.index, pd.MultiIndex):
            datetime_level = combined_df_multi.index.get_level_values(0)
            if not isinstance(datetime_level, pd.DatetimeIndex):
                datetime_level = pd.to_datetime(datetime_level)
            # Rebuild MultiIndex with proper Timestamp type and correct names
            combined_df_multi.index = pd.MultiIndex.from_arrays(
                [datetime_level, combined_df_multi.index.get_level_values(1)],
                names=['datetime', 'instrument']
            )
            combined_df_multi = combined_df_multi.sort_index()
            logger.debug(f"  Dataset index fixed: dtype={combined_df_multi.index.get_level_values(0).dtype}, names={combined_df_multi.index.names}")
    ...
    def _train_and_backtest(self, dataset, exp_name: str, rec_name: str, output_name: Optional[str] = None) -> Dict:
        ...
        with R.start(experiment_name=exp_name, recorder_name=rec_name):
            ...
            # Fix: Ensure MultiIndex datetime level is Timestamp type and names are correct
            if isinstance(pred.index, pd.MultiIndex):
                datetime_level = pred.index.get_level_values(0)
                # Convert to Timestamp if not already
                if not isinstance(datetime_level, pd.DatetimeIndex):
                    datetime_level = pd.to_datetime(datetime_level)
                # Rebuild MultiIndex with proper Timestamp type and correct names
                pred.index = pd.MultiIndex.from_arrays(
                    [datetime_level, pred.index.get_level_values(1)],
                    names=['datetime', 'instrument']
                )
                # Sort index to avoid slicing issues
                pred = pred.sort_index()
                logger.debug(f"  Pred index fixed: dtype={pred.index.get_level_values(0).dtype}, names={pred.index.names}")
