...
export interface BacktestHistoryDetailResponse {
  ...
  metrics: {
    ...
    __factor_source?: string;
    __num_factors?: number;
    __elapsed_seconds?: number;
  };
}

export async function listBacktestHistory(limit?: number) {
  const qs = new URLSearchParams();
  if (limit) qs.set('limit', String(limit));
  return request<BacktestHistoryResponse>(`/api/v1/backtest/history/list?${qs.toString()}`);
}

export async function getBacktestHistoryDetail(filename: string) {
  const qs = new URLSearchParams();
  qs.set('filename', filename);
  return request<BacktestHistoryDetailResponse>(`/api/v1/backtest/history/detail?${qs.toString()}`);
}
