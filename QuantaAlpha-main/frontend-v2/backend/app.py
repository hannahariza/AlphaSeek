...
@app.get("/api/v1/backtest/history/list", response_model=ApiResponse)
async def list_backtest_history(limit: int = Query(20, ge=1, le=100)):
    """
    List historical backtest results from the output directory.
    Returns a list of backtest summary items sorted by timestamp (newest first).
    """
    try:
        ...
        if config_path.exists():
            ...
            output_dir_raw = "data/results/backtest_v2_results"
        ...
        if not output_dir.exists():
            return ApiResponse(success=True, data={"history": [], "total": 0})
        ...
        for mf in metrics_files[:limit]:
            try:
                ...
                history.append({
                    ...
                    "timestamp": os.path.getmtime(mf),
                    "datetime": datetime.fromtimestamp(os.path.getmtime(mf)).isoformat(),
                    "experiment_name": data.get("experiment_name", ""),
                    "factor_source": data.get("factor_source", ""),
                    "num_factors": data.get("num_factors", 0),
                    "metrics": {
                        "IC": metrics.get("IC"),
                ...
                logger.warning(f"Failed to load backtest history item {mf}: {e}")
                continue
        
        return ApiResponse(success=True, data={"history": history, "total": len(history)})
    
    except Exception as e:
        return ApiResponse(success=False, error=f"Failed to list backtest history: {str(e)}")


@app.get("/api/v1/backtest/history/detail", response_model=ApiResponse)
async def get_backtest_history_detail(filename: str = Query(..., description="Backtest metrics JSON filename")):
    """
    Load detailed backtest results including cumulative curve for a specific historical backtest.
    """
    try:
        # Determine output directory from config
        config_path = PROJECT_ROOT / "configs" / "backtest.yaml"
        ...
        metrics_file = output_dir / safe_filename
        
        if not metrics_file.exists():
            raise HTTPException(status_code=404, detail=f"Backtest result not found: {filename}")
        
        # Load metrics JSON
        with open(metrics_file, "r", encoding="utf-8") as f:
            data = json.load(f)
        
        # Flatten metrics for frontend
        inner_metrics = data.get("metrics", {})
        flat = {**inner_metrics}
        
        # Add metadata
        for key in ("experiment_name", "factor_source", "num_factors", "config", "elapsed_seconds"):
            if key in data:
                flat[f"__{key}"] = data[key]
        
        # Load cumulative curve from CSV if available
        csv_path = metrics_file.with_name(safe_filename.replace("_backtest_metrics.json", "_cumulative_excess.csv"))
        if csv_path.exists():
            try:
                import pandas as pd
                ...
                logger.warning(f"Failed to load cumulative curve for {filename}: {e}")
        
        return ApiResponse(success=True, data={
            "filename": safe_filename,
            "timestamp": os.path.getmtime(str(metrics_file)),
            "datetime": datetime.fromtimestamp(os.path.getmtime(str(metrics_file))).isoformat(),
            "metrics": flat,
        })
        ...
        return ApiResponse(success=False, error=f"Failed to load backtest detail: {str(e)}")
